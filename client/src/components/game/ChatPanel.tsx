...
export function ChatPanel() {
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 ${chatOpen ? '' : 'pointer-events-none sm:pointer-events-auto'}`}>
      {/* Collapsed chat button.
          Hidden on phones: a fixed bottom-right button sits directly on top of
          the player's hand there, swallowing taps and card drags. On small
          screens the toggle lives in the RoundInfo header instead. */}
      {!chatOpen && (
        <button
          onClick={() => setChatOpen(true)}
          className={`relative hidden sm:flex items-center gap-2 px-4 py-2 rounded-full ${isLight ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-emerald-700 hover:bg-emerald-600'} text-white shadow-lg transition-colors`}
        >
